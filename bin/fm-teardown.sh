...
# (/tmp/fm-<uid>-<id>/); absent for tasks spawned before that change, so tolerate empty.
...
# Remove the per-task temp root (/tmp/fm-<uid>-<id>/, incl. its gotmp/) recorded by spawn.
